...
for rel in "${!files[@]}"; do
  ...
  if [ "$rel" = "docs/TASKS.md" ]; then
    entries+=("    \"$rel\": {\"line_count\": $line_count, \"active_task_count\": $active_count, \"measured_at\": \"$measured_at\"}")
  else
    entries+=("    \"$rel\": {\"line_count\": $line_count, \"measured_at\": \"$measured_at\"}")
  fi
  print_info "$rel: lines=$line_count"
done

# 用 printf 拼接：避免 heredoc 把字面 \n 当作字符
joined=$(IFS=,; printf '%s' "${entries[*]}")
baselines_json=$(printf '{\n%s\n  }' "$joined")
...
state_tmp=$(mktemp)
{
  ...
  printf '  "first_run_at": "%s",\n' "$measured_at"
  printf '  "last_scan_at": "%s",\n' "$measured_at"
  printf '  "multiplier": 1.5,\n'
  printf '  "baselines": %s,\n' "$baselines_json"
  printf '  "history": []\n'
  printf '}\n'
} > "$state_tmp"
