...
function settingsWithRawPaddleKey(): AppSettings {
  ...
  // 与生产 SettingsPanel 容器一致：进入 section 前先 exportSafe 脱敏 apiKeyRef 占位。
  return exportSafeAppSettings(settings);
}

describe("OcrProviderSection", () => {
  test("renders default backend, provider list and masked API key placeholder", () => {
    render(<OcrProviderSection settings={settingsWithRawPaddleKey()} onChange={() => undefined} />);
  ...
  test("edits provider enablement, endpoint, key reference and network confirmation", async () => {
    ...
    const lastChange = vi.fn<(settings: AppSettings) => void>();
    render(
    ...
    const lastSettings = lastChange.mock.calls.at(-1)?.[0];
  ...
  test("switches default OCR provider via select", async () => {
    const user = userEvent.setup();
    const lastChange = vi.fn<(settings: AppSettings) => void>();
    render(
      <ControlledHarness initial={createDefaultAppSettings()}>
    ...
    await user.selectOptions(screen.getByLabelText("默认 OCR 后端"), "paddleocr");

    const lastCall = lastChange.mock.calls.at(-1)?.[0];
    expect(lastCall?.defaultOcrProviderId).toBe("paddleocr");
  });
});
