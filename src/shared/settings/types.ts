...
/** ISS-NEW-G（2026-06-22 收口）：PDF Expert 打开方式偏好（双击 PDF 时路由）。 */
export type PdfExpertOpenMode = "always-pdf-expert" | "system-default" | "ask-each-time";
...
export interface AppSettings {
  ...
  /** ISS-NEW-G（2026-06-22 收口）：PDF Expert 打开方式偏好。默认 "ask-each-time"。 */
  pdfExpertOpenMode: PdfExpertOpenMode;
  /**
   * ISS-NEW-G（2026-06-22 收口）：重新打开 PDF 时是否回到上次阅读位置。
   * 对齐 PDF Expert Preferences「回到页面」（number input 解读为 toggle，留 TODO 解析具体 number）。
   * 默认 true（用户体验友好）。
   */
