...
> ISS-030 ~ ISS-055 批量完成：工具栏克制化、UI 视觉统一、空态清理、菜单中文化、设计系统重构、多层级工具启动器、导出交付面板、表单 / 批注扁平化入口收口、页面管理真实状态接入、阅读渲染稳定性修复、工具启动器去假入口、阅读工具注册幂等修复、原生菜单系统动作收口、页眉页脚范围和位置深化、左侧摘要状态标记克制化、深色模式最小接入和顶栏任务模式入口收口。

UI 改进：

- 工具栏克制化（ISS-030 / ISS-037）：48px 高度、移除品牌区域、布局按钮收左上角 compact 图标模式、侧边栏默认关闭
- 顶栏任务模式入口收口（ISS-055）：`OCR / 批注 / 填写和签名 / 导出` 不再平铺在阅读态顶栏，统一从右侧 `工具` 工作流启动器进入；切入后仍显示对应上下文工具条或工作台
- PDF 工具启动器（ISS-039）：右侧 `工具` 按钮打开 `组织页面 / 交付导出 / 标注填写 / 扫描 OCR` 四组菜单，低频命令不再平铺到阅读态顶栏
- 导出交付设置面板（ISS-039 / ISS-040 / ISS-041 / ISS-042 / ISS-051 / ISS-053）：`添加页码` / `Bates 编号` / `页眉页脚` / `压缩` 从三级工具入口进入导出模式右侧面板，二级导出工具条只保留 `文字水印 / 图片水印`；页眉页脚参数支持 `全部页面 / 奇数页 / 偶数页` 应用范围，页眉和页脚各自支持左 / 中 / 右视觉位置选择
- 填写签名工具条收口（ISS-043）：`表单扁平化` 从三级工具 / 原生菜单进入填写和签名面板；填写签名二级工具条只保留 `读取字段 / 填写 / 签名 / 扁平化导出`，移除日期、钩号等未接通占位按钮
- 页面管理工作台收口（ISS-044）：页面管理继续作为左上角独立工作台，只展示已接通的 `旋转 / 删除 / 撤销 / 另存为新 PDF`，移除插入、复制、摘录等未接通占位按钮
- 页面管理重排 UI（ISS-046）：页面管理工作台新增 `上移 / 下移`，边界页禁用无效移动；重排不进入阅读态顶栏或导出二级工具条
- 批注扁平化入口收口（ISS-045）：`批注扁平化` 归入 `工具 > 标注填写` 和原生工具菜单，触发后进入批注模式并打开批注侧栏，不进入阅读态顶栏或导出二级工具条
- 工具启动器去假入口（ISS-048）：空态移除未接通的图片 / Word 转 PDF 按钮；`批注摘要` 改入 `工具 > 标注填写` 并打开批注侧栏摘要视图；`另存为` 保留在 `交付导出` 分组但执行真实副本保存
- 阅读工具注册幂等修复（ISS-049）：重复初始化或开发热更新后，阅读辅助按钮 `逆时针 / 顺时针 / 适合页面` 不再重复出现在顶栏
- 原生菜单系统动作收口（ISS-050）：`文件 > 新建窗口` 直接创建新的 FaroPDF 窗口；`帮助 > 关于 FaroPDF` 直接打开设置页的 `关于` section，不再显示占位提示
- 左侧摘要状态标记（ISS-052）：缩略图页码上的批注 / 搜索命中 / OCR 状态改为紧凑视觉点，保留悬停说明和可访问名称，减少长卷宗左侧重复文字
- 深色模式（ISS-054）：`设置 > 常规` 新增 `外观` 偏好，支持浅色 / 深色切换；主题写入根节点 `data-theme`，工具栏、设置浮层、面板和状态栏共享深色 token，PDF 纸面不反相
- 空态顶栏收口（ISS-039）：未打开文档时隐藏旋转 / 适合页面等文档专属辅助按钮，页码显示改为 `- / -`
- 欢迎页空态清理（ISS-034）：移除硬编码占位文件名和最近文件区域
- 设置页视觉统一（ISS-035 / ISS-054）：所有控件 focus-visible 使用 accent 体系、清理遗留 CSS 冲突；常规页承载浅色 / 深色外观偏好
- 主页灰色区域修复（ISS-033）：阅读区 flex: 1 填满可用空间

功能修复：

- PDF 拖拽打开（ISS-031）：阅读态 DocumentReader 支持 DnD、PDF.js worker 幂等配置、渲染错误日志
- PDF.js canvas 渲染取消（ISS-047）：页面重新渲染、模式切换或组件卸载时取消上一轮 render task，避免同一 canvas 并发 render 后回退到占位页
- 页码 / Bates 导出（ISS-039）：普通编号输出 `*-page-numbered.pdf`，证据编号输出 `*-bates.pdf`，均通过浏览器下载新 PDF，不覆盖原始文件
- 文字 / 图片水印导出（ISS-040）：文字水印支持内容、位置、字号、透明度和旋转；图片水印支持 PNG/JPG、位置、宽度和透明度，默认输出 `*-text-watermarked.pdf` / `*-image-watermarked.pdf` 新副本
- 页眉页脚导出（ISS-042 / ISS-051 / ISS-053）：页眉页脚支持页眉、页脚、应用范围（全部页面 / 奇数页 / 偶数页）、页眉 / 页脚位置、字号和透明度；无匹配页面时阻止导出并提示，默认输出 `*-header-footer.pdf` 新副本
- 压缩导出（ISS-041）：压缩设置接入同一个右侧交付面板，支持法院上传 5MB / 10MB / 20MB / 50MB 等预设，默认输出 `*-compressed.pdf` 新副本；导出引擎的压缩 apply 模式会使用压缩服务输出替换工作 PDF
- 表单扁平化入口（ISS-043）：深层 `表单扁平化` 命令会打开填写和签名面板，由面板承载读取字段、状态提示和 `*-flattened.pdf` 新副本导出确认
- 页面整理另存（ISS-044）：页面管理工作台接入 `pageOrganizer` 状态机，旋转、删除和撤销会真实更新页面网格；另存使用 `page-operations execute` 输出 `*-organized.pdf` 新副本，不覆盖原始 PDF
- 页面重排另存（ISS-046）：页面管理工作台通过 `reorderOrganizerPages` 更新页序，撤销可恢复；另存继续使用 `page-operations execute` 写入真实页序并输出 `*-organized.pdf` 新副本
- 批注扁平化导出（ISS-045）：批注侧栏在有批注时显示 `扁平化导出`，调用 `flatten-annotations draw` 真实绘制批注，默认保存 `*-annotations-flattened.pdf` 新副本；无批注时不显示假入口
- 另存为副本（ISS-048）：`工具 > 另存为` 读取当前 PDF 源 bytes 并保存 `*-copy.pdf` 新副本；缺少源 bytes 时给出明确错误，不显示占位成功提示

本地化：

- macOS 菜单栏中文化与桥接（ISS-032 / ISS-050）：文件/编辑/视图/工具/窗口/帮助使用 Tauri v2 MenuBuilder；工具菜单通过 `faropdf://command` 进入前端 command model，`文件 > 打开…` 走 Tauri dialog + 专用 PDF bytes 读取 command；新建窗口 / 全屏 / 关闭窗口由桌面壳层直接处理

设计系统：

- DESIGN.md 重构为 21 节成熟结构（ISS-038）：新增组件样式、信息密度、交互规则、深度层级、响应式、空态规范、工具栏克制原则、设置页规范、菜单栏规范、禁止事项、设计评审等章节

## Unreleased (continued) — 3 阶段排查与 P1 视觉偏差修复

> 2026-06-14 用户反馈"功能没真正完成、布局没按 PDF Expert 来、一直在做错误的功能"后,按"先排查再修复"原则开展 3 阶段排查（静态扫描 → Chrome headless 截图 → Playwright 精细验证）。报告存于 `docs/plans/2026-06-14-faropdf-audit-stage-{1,2,3}-report.md`。

可访问性：

- Toolbar 左区 3 个 compact 图标按钮补 `aria-label`（ISS-FIX-1 / DEC-094）：`src/components/layout/Toolbar.tsx` 行 71/80/89 三处加 `aria-label="文档摘要"` / `aria-label="页面管理"` / `aria-label="视图设置"`, 此前仅有 `title` 属性, 屏幕阅读器表现不一致, 1x 视口下 OCR 也无法识别。

排查结论（用户可见的偏差确认）：

- 第 1 阶段静态扫描发现 5 个 P0（OCR 是 stub / 图片转 PDF 战略层缺位 / 压缩只跑对象流 / 批注图章导出只画矩形+文字 / 微信二维码 1×1 占位）+ 11 个 P1 视觉偏差。
- 第 3 阶段 Playwright 实证：F-04 微信二维码**已通过 DEC-062 流程替换为真实 734×734 图片**, 第 1 阶段静态判断错误; 2.3.2 / 2.4.13 / 2.4.14 / 2.4.19 / 2.4.30 / 2.5.1 六个 P1 视觉偏差全部复现, 2.5.1 为新增。

ISS-NEW-E ROADMAP 状态对齐（DEC-096）：

- `docs/ROADMAP.md` §5 行 70"证据图片 A4 多图编排"原标 [ ] 与 README §导出 / CHANGELOG 0.1.0-alpha.1 / 0.1.0-alpha.6 ISS-018 实际交付描述矛盾, 改为"**部分**：[x] plan-only 编排计划器 / [ ] 真实目录拾取和像素渲染", 加 ISS-018 / DEC-019 指针。
- `docs/ROADMAP.md` §6 行 85"扫描件清洁校正"原标 [ ] 与 ISS-016 / DEC-040 preprocess-only job/bridge 真实清洁已落矛盾, 改为"**部分**：[x] preprocess-only 真实清洁 / [ ] 粗方向 / 微倾斜 / 拆分双页 plan-only", 加 ISS-016 / DEC-040 指针。
- `docs/ROADMAP.md` §7 行 97"签名图片 / 手写签名 / 签名位置调整"原标 [ ] 与 README §表单签署 / CHANGELOG 0.1.0-alpha.5 ISS-008 实际交付描述矛盾, 改为"**部分**：[x] 签名图片 PNG/JPG / [ ] 手写签名 / 签名位置调整", 加 ISS-008 / DEC-055 指针。
- §5 行 66-67"插入 PDF / 合并 / 提取页码范围" 整组缺位保留 [ ] 状态, 留作 ISS-NEW-A 另立, 不在本期文档同步范围。
