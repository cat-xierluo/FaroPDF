...
### Fixed
- **真机文字选不中三层修复 +「真实拖选」门禁（ISS-QA-24，2026-08-15）**：文字层落地（ISS-QA-18）且产物层断言全过后，真机仍反馈「无法选中文字」——产物层诊断一切正常（38 span、user-select auto、程序化框选成功），矛盾根源是**程序化 Range 选区不经过 hit-testing**，证明不了用户鼠标真能选。三个确定性缺陷：① single/double 模式下整页 click 都当翻页（`handlePageClick`）——拖选文字松手必触发 click→翻页，页面切换立即销毁选区，且点文字本身也翻页；现双守卫：selection 非空不翻页 + 点击落在文字层上不翻页（点空白才翻）。② `usePdfTextSelection` 监听 selectionchange 每次都 setBounds → 整棵 AppShell 重渲染，拖动中每 mousemove 一次——真机 WKWebView 上拖选卡顿/「选不动」；现拖选进行中（mousedown→mouseup）抑制 setState + 值等价跳过，顺带修了点击空白后工具条残留（collapsed/0×0 选区现在正确收起）。③ `.pdf-text-layer` 及其 span 显式 `-webkit-user-select/user-select: text`（CSS 规范里 auto 在祖先为 none 时计算为 none，显式声明消除宿主全局样式/引擎继承差异）+ canvas/page-container 显式 `z-index:0` 防御 WKWebView 堆叠规则差异。门禁升级：`verify:prod-render` 新增**真实拖选断言**——chromium CDP `mouse.down→分步 move→up` 走原生选择路径（实跑从标题拖到正文选中 387 字符），从此「能不能选中」类验收不再依赖程序化 Range（它抓不到「覆盖层挡鼠标 / click 翻页销毁选区」类 bug）；ReaderCanvas 补翻页守卫单测 3 条。真机 WKWebView 待用户实测确认。
...
### Added
- **侧栏批注删除（ISS-QA-22，2026-08-15）**：AnnotationSidebar 行 hover 时显示 Trash2 按钮（CSS `:has(:hover/:focus-visible)` 透明→可见），点击 native confirm 后调 `onDeleteAnnotation(id)` → App.tsx 调 `AnnotationService.deleteAnnotation` 并过滤本地列表。嵌套 button 用 span+role+onKeyDown 避免非法 HTML（Enter/Space 触发）。
- **Overlay↔Sidebar 双向闭环（ISS-QA-23，2026-08-15）**：点 overlay glyph → `setActiveAnnotationId` → 双 RAF 后用 `[data-annotation-row-id]` 选择器把 sidebar 行 `scrollIntoView({block:"nearest"})` 滚入视口（glyph 已有的 `[data-annotation-id]` 选择器复用同一 try/catch 链）。点 sidebar 行已有 `scrollIntoView` 滚 glyph（ISS-QA-21）——现两侧闭环，验证：单测「点击 Sidebar 批注 → scrollIntoView 滚 glyph + sidebar 行」断言 spy 收到 ann-page1 命中。
...
- **Retina 屏渲染模糊修复（ISS-QA-17）**：`renderPageToCanvas` 此前把 canvas backing store 按 CSS 像素 1x 渲染，Retina（devicePixelRatio=2）上被浏览器拉伸显示——「渲染成模糊图片」。现 backing store 按 `devicePixelRatio` 放大 + pdfjs render `transform: [dpr,0,0,dpr,0,0]` 同步缩放，CSS 尺寸钉在 viewport。`verify:prod-render` 门禁新增 DPR=2 场景断言（backing = css × dpr），此 bug 类从此被机械拦截（chromium 默认 DPR=1 复现不了）。
- **产品定位文案统一「面向知识工作者」收尾（ISS-QA-15）**：`package.json` description、README 定位行、关于界面、WelcomeScreen 此前已切换；本轮清除 README 功能清单最后一处「大卷宗」（→「大型文档」）。作者署名「杨卫薪律师」为个人信息非产品定位，保留。至此用户可见 surface（`src/` UI 文案 grep 零残留）全部统一。
...
### Added
- **全局键盘快捷键（2026-08-15）**：`useGlobalHotkeys` hook 在 AppShell 挂载：⌘/Ctrl+F 聚焦搜索（⌘/Ctrl+Shift+F 不触发聚焦，留给浏览器原生「页面内查找」语义）；⌘/Ctrl+[ 上一命中、⌘/Ctrl+] 下一命中（直接接 search controller selectPreviousHit / selectNextHit）；Esc 在任意上下文关闭搜索面板（覆盖可编辑元素，确保弹窗/选区/搜索面板都能 Esc 关）；input/textarea/contenteditable 内不拦截 ⌘F / ⌘[ ]（避免编辑冲突，Esc 仍响应）。跨平台识别 macOS `metaKey` 与其它平台 `ctrlKey`。Toolbar 搜索框补 `data-testid="toolbar-search-input"` 供 E2E 断言。验证：单测 5（⌘F 聚焦、Ctrl+F 聚焦、Esc 任意上下文、⌘[ ] 切、可编辑跳过 + ⌘⇧F 不触发）；`verify:prod-render` 真实 UI 断言 ⌘F 后 `document.activeElement.testid === "toolbar-search-input"` PASS；全量 1497 单测全绿。
- **真实搜索高亮矩形 — 页内命中按坐标画半透明矩形（2026-08-15）**：搜索命中此前在页面上以文字 chips 占位（「搜索命中：xxx」标签列）；现用 pdfjs 原始 text item 几何（transform/width/height）计算命中包围盒，在页面内按 `pt × zoom` 绝对定位渲染半透明高亮矩形（`--selection` 30% tint，垫在文字层之下）。`computeTextItemRects` 纯函数（search 模块，结构化参数零反向依赖）+ `LoadedPdfDocument.findTextRects`（组合 `getPageViewport(pt)` 与 raw text content）+ controller 委托 + `ReaderCanvas` 逐页渲染（query/命中页变化刷新，zoom 变化只重定位）。v1 为 item 级矩形（pdf.js viewer 字符级推进留后续）；跨 item 命中仍进结果列表、页内不画矩形。验证：纯函数单测（y 翻转/大小写/缺几何跳过）；`test:e2e` 真 pdfjs 几何断言（pt 域界内）；`verify:prod-render` 真实 UI 驱动（输入 MUTUAL → 页内 771×38px 定位矩形）。
- **阅读区 pdfjs 文字层 — 文字可选中、可复制（ISS-QA-18）**：此前阅读区只渲染 canvas 位图，内容无法选中复制（`TextSelectionToolbar` 在等一个从未渲染的文字层）。现 `PdfPage` 在 canvas 上叠一层 pdfjs `TextLayer`（透明可选中的文字 span）：`pdfReaderService.renderTextLayer`（`--total-scale-factor` = viewport.scale，与 canvas CSS 尺寸同域对齐；Retina backing 的 DPR 放大不影响本层，文字天然矢量清晰）→ `useReaderController.renderTextLayer` → `AppShell` → `ReaderCanvas` 逐页渲染（zoom/rotation 变化重渲，abort 跟随）。CSS 对照 pdf.js 官方 `.textLayer` 裁剪适配（透明文字 + `::selection` 用 `--selection` 35% tint）。`textLayerStatus ≠ available`（纯扫描）不渲染该层，渲染失败 fail-closed（console.warn + 清空，阅读不受影响）。门禁：`verify:reader-e2e` 加 TextLayer 断言（16 span + 真实文本）；`verify:prod-render` 加真实 UI 程序化框选断言（38 span，`window.getSelection()` 取到「MUTUAL NON-DISCLOSURE AGREEMENT」）。ISS-061 的选区浮动工具条（高亮/下划线/复制等 7 动作）自此有真实选区可用。
- **verify:prod-render 产物层门禁 + 前端构建戳（DEC-200）**：① 新增 `npm run verify:prod-render`（`scripts/verify-prod-render.mjs`）：vite preview（**build 产物**，非 dev server）+ chromium 真实 UI 打开 PDF → canvas 像素 + 「文字层可用」断言，外加 **Retina DPR=2 场景**（backing = css × dpr）——补齐既有三层门禁都不碰 build 产物的盲区；② vite define `__FAROPDF_BUILD_ID__` 构建戳 + 启动 console 打出：tauri 资产嵌入是压缩的，静态 grep 无法判产物新旧（实测二进制同时含新旧资产哈希），构建戳让真机 devtools 一眼确认嵌入版本——本轮真机排查第 1 轮即靠它排除「产物新旧不明」，拿到决定性 console。
...
### Known Issues
- **真机文字层选不中（ISS-QA-24 代码已修复，待真机确认）**：文字层 DOM 已落地（ISS-QA-18），本地全门禁含新「真实拖选」断言全绿；打包 WKWebView 上用户此前仍选不中——本轮三层修复（翻页守卫 / 拖选重渲染抑制 / 显式 user-select）已入库并重建，待用户硬刷新后实测。若仍失败需 devtools Elements（确认 `.pdf-text-layer` span 存在）+ Console 红字定位。

### Fixed
- **PDF 文字层「未知」修复 — standardFontDataUrl 第 2 版（ISS-QA-02）**：workerPort 修复（`97f3056`）后 PDF 已能正常打开，但状态栏仍「文字层未知」——真机 WKWebView 下 `page.getTextContent()` 抛 `UnknownErrorException: Ensure that the standardFontDataUrl API parameter is provided`，被 `readTextLayerStatus` catch 吞错。`acc488e` 第 1 版用 `new URL("pdfjs-dist/standard_fonts/", import.meta.url).href`，但 Vite **不把 bare module 目录当资产**，打包后解析为 `tauri://localhost/assets/pdfjs-dist/standard_fonts/` → 404 无效。现第 2 版：`config/vite.config.ts` 新增 `provideStandardFonts` plugin（dev 中间件把 `/standard_fonts/*` 映射到 node_modules 真实字体 + 防目录穿越；build `closeBundle` 复制 16 个字体到 `dist/standard_fonts/`），代码侧 `standardFontDataUrl: "/standard_fonts/"` 根路径 dev/prod 一致。验证：chromium 真 module worker 下 reference.pdf `textLayerStatus="available"`（pageCount=5 / 提取 422 字符）、corrupt.pdf 抛 `InvalidPDFException`；`npm run build` 后 `dist/standard_fonts/` 实有 16 文件 + curl 200；typecheck / lint(0 error) / reader-renders e2e(7/7) 全绿。同时清理 `readTextLayerStatus` catch 内遗留的 `alert("QA-02 DIAGNOSTIC…")` 调试桩（保留增强版 `console.warn`）。**NOT_VERIFIED**：WKWebView 真机实拍（Safari 开发菜单列不出 FaroPDF target），留用户 `npm run tauri build` 后实机确认「文字层：可检测」。另：`Cargo.toml` + `tauri.conf.json` 开 `devtools`（上个 session 为 WKWebView 调试加，release 也会生效，后续稳定后可关）。
- **阅读器加载失败可观测性（ISS-QA-02 诊断，非盲目改 worker）**：打开 PDF 显示「损坏 / 未知」的真 pdfjs error 此前被 `normalizeError` 归一化吞掉（`InvalidPDFException`→「文件可能已损坏」），真机 devtools 看不到根因，导致上一轮 workerPort 改动（基于 W2 已被 REFUTED 的 worker 假设）实机仍崩、陷入盲改循环。现补三层 console 诊断：① `pdfjsWorker.ts` 给 Worker 加 `error` 事件监听 + 构造 try/catch，构造 / 异步加载失败（404 / MIME / scheme）打印真因后降级 fake worker（不抛）；② `pdfReaderService.ts` `loadPdfFromBytes` 关键抛错点 `loadingTask.promise` 包 try/catch，reject 时 `console.error` 真 pdfjs error（name / message / stack）再 rethrow；③ `readTextLayerStatus` 的静默 `catch` 改 `console.warn`。真机 devtools 现可直接定位「打开 PDF 显示损坏」背后是 Worker 加载失败 / InvalidPDF / 加密 哪一类，替代盲改。
- **阅读器渲染 e2e 回归门禁（verification-loop 阶段 5 + ISS-QA-02 复现测试）**：新增 `tests/e2e/reader-renders.test.ts`（vitest 集成，follow 既有 `forms-e2e` / `ocr-e2e` 模式）+ `package.json` `test:e2e` 脚本。用真 pdfjs（`pdfjs-dist/legacy/build/pdf.mjs`，node 兼容）+ 真 fixture（`expert/reference.pdf` + `reader/corrupt.pdf`）驱动 `loadPdfFromBytes` 完整链路（getDocument→getPage→getViewport→getTextContent），断言正常 PDF 加载成功（pageCount=5 + textLayerStatus≠unknown + 有文字）、损坏 PDF 抛错。**决定性诊断结论**：该测试在 vitest/jsdom 下 PASS = `loadPdfFromBytes` 的 pdfjs 调用逻辑无 bug，QA-02 真根因锁定在 Tauri WKWebView 环境特有（`tauri://` scheme 下 worker 加载链路失败），而非 pdfjs API 逻辑——也纠正了此前「真因是 readTextLayerStatus 的 getTextContent」的误判（它 catch 吞错不影响 loadPdfFromBytes 抛出，且 UI 不显示 textLayerStatus）。注：jsdom 无真 Worker，测试用自定义 adapter（legacy build + `file:` scheme workerSrc）绕 configureWorker；真机 WKWebView 的 Worker 行为本测试不覆盖（覆盖层 = 真机验证，见 verification-loop lessons-from-practice 教训 3）。
- **OCR 默认设置测试同步 ISS-QA-16（回归修复）**：`defaults.test.ts` 仍断言 MinerU 默认 endpoint 为空，但 ISS-QA-16 已把 MinerU 默认 endpoint 预填为 `https://mineru.net/api/v4`（enabled 仍 false，需用户显式启用 + 配 apiKey），测试未跟上导致回归失败。现断言更新为官方默认地址，disabled 语义仍由同测试内 `requiresNetworkConsent + enabled=false` 守。verification-loop 阶段 4 一次性抓到 3 个历史 test 回归（另 2 个 `App.test.tsx`：menuitem 批注/导出 找不到、`reader.reportOpenError is not a function`，Wave 3 / QA-12 / QA-01 引入，已登记 ISS-QA 待后续修）。
- **设置页 + 关于界面视觉参照 Folia，去光泽（ISS-QA-13 / ISS-QA-14）**：设置页左侧导航「当前选项」选中态此前整行铺满 `--accent-soft`（`#d9eef0`，饱和淡青色块），观感像带光泽的玻璃块、刺眼难看。参照 Folia 设置页（`.settings-nav-item.active`）改为「左侧 accent 竖条（`box-shadow: inset 2px 0 0 var(--accent)`）+ accent@10% 极淡 tint（`color-mix(in srgb, var(--accent) 10%, transparent)`）+ `--fg` 文字」，选中语言由形态承担、不再灌满饱和色；hover 同步由饱和 `accent-soft` 改为中性极淡灰 wash（`var(--fg) 5%`）；窄屏顶部 tab 用同款 tint + 下边 accent 条（呼应 Folia `.tabbar-tab--active`）。关于界面（ISS-QA-14）参照 Folia 扁平化：产品卡背景由 stark `--surface-strong` 换为柔和 `--surface`、产品名字重 `700→600`；版本 / 更新状态信息行由含糊的 grid 改为 Folia `.about-info-row` 同款的「横向 label/value + 行底分隔线」info-panel（顶/底 `--border-soft` 分隔）；作者区由独立卡片盒改为顶部 `--border-soft` 分隔线 + 透明背景的扁平区块，消除「双卡片」叠加感。**只改 `SettingsPanel.css`（视觉参照，不照搬 Folia 代码/类名），不动 `--accent-soft` token（app.css ~14 处复用）、不重构 JSX、不改 scope 外的 `AuthorCard.css`**。`color-mix` 已见于 app.css L1670、Tauri WebView 支持，且自动适配 dark 主题 accent。`npm run typecheck` 通过；GUI 实机视觉确认留用户。

- **工具栏「+」二级菜单去重 + 批注印章浮层归位（ISS-QA-12 / QA-09 / QA-07，本轮范围内）**：用户实机反馈「+」launcher 功能多、分类乱、与其他页重复，导出二级菜单怪，以及批注工具浮层脱离。本轮**只做最小修，不重写 Toolbar 架构**：① **ISS-QA-12 去重**——`Toolbar.tsx` 的 `ToolLauncherMenu` 在渲染层过滤掉与 L3 工作流段重复的 `mode-*` 切换（批注/导出/填写签名/扫描）和导航段「页面管理」（`view-pages`），它们已是工具条一级入口，二级菜单不再重复；去重后空段（organize/scan）自动跳过。② **ISS-QA-09 受益**——`mode-export` 被去掉后，deliver 段回归纯导出工具（另存/水印/页眉页脚/页码/Bates/压缩/密码），不再混杂 mode 切换项。③ **ISS-QA-07 印章浮层**——`AnnotationToolbar.css` 给 `.annotation-toolbar-options` 补 `position:relative`，作为 `.annotation-toolbar-stamp`（`position:absolute`）的定位祖先，印章模板浮层现锚定到工具条正下方居中，不再脱离到视口顶部居中。**关键约束**：`APP_TOOL_LAUNCHER_SECTIONS` 数据结构被 `commands.test.ts`（不在本轮可改范围）按段 id 与成员锁定，故去重在**渲染层**实现、`commands.ts` 数据不动——`commands.test.ts` 的段结构断言不受影响，`Toolbar.test.tsx` 的「翻译」menuitem 仍渲染（非 mode 项未过滤）。`npm run typecheck` 通过。
...
- **填写/签名两处签名预览居中一致（ISS-QA-10）**：用户实机反馈「填写和签名」面板与「签名」右栏的签名缩略图视觉不齐。根因是内嵌在填写和签名面板 `SignatureEditor` 里的 `SignatureLibraryPicker`（`.signature-library-picker__*`）此前**完全无 CSS**，缩略图按 img 原始尺寸渲染、未居中、尺寸参差；而 `SignaturePanel`（signatures 右栏，`.signature-panel__thumbnail`）已有固定 60px 高 + `object-fit:contain` 居中。两处居中手法对齐到同一套：新增 `SignatureLibraryPicker.css`（flex-wrap 行 + 固定 72×40 缩略图盒 + `align/justify center` + `object-fit:contain`），并把 `SignaturePanel` 缩略图由 `display:block`（inline img 易留 baseline 缝隙）改为 `display:flex` 居中、img 由 `width/height:100%` 改为 `max-width/max-height:100%`，消除基线缝隙、保证水平垂直绝对居中。`SignatureLibraryPicker.tsx` 补 CSS import。`npm run typecheck` 通过；GUI 实机居中走查留用户。
...
- **拖入 PDF 打开（ISS-QA-01）**：从系统 Finder 把 `.pdf` 拖入 FaroPDF 窗口此前完全无反应——前端三处拖拽（`ReaderCanvas` / `WelcomeScreen` / `ReaderErrorScreen`）走 HTML5 `onDrop` + `event.dataTransfer.files`，而 Tauri v2 webview 中系统文件拖拽不进入 `dataTransfer.files`，必须由 Rust 监听 `WindowEvent::DragDrop`。现 `lib.rs` Builder 补 `.on_window_event`，匹配 `WindowEvent::DragDrop` → `DragDropEvent::Drop { paths, .. }`，过滤首个扩展名 `.pdf`（大小写不敏感，非 PDF 一律忽略），`emit_to(EventTarget::labeled(window.label()))` 自定义事件 `faropdf://file-drop`（payload `{ path }`），限定到接收拖拽的窗口避免多窗口串扰；`Enter`/`Over`/`Leave` 忽略。前端 `App.tsx` 顶层 `listen("faropdf://file-drop")` 取路径 → 复用既有 `readPdfFileFromPath`（路径→bytes，Rust `read_pdf_file_from_path` 已存在）→ `reader.openNativeFile` 打开，失败走与原生菜单「打开」一致的归一化中文错误态（`reportOpenError`）。tab 注册由 `AppShell` 既有 effect（监听 `reader.state.document`）自动完成，无需新增 openTab 调用。`readPdfFileFromPath` 前端入口（`tauriPdfFileService.ts`）已具备，无需补。HTML5 `onDrop`（三处）保留作 web 降级，未删未改。GUI 实机拖拽验证留用户。
- **工作流 mode 入口展开 + stub fail-closed（ISS-QA-04）**：批注 / 编辑 / 导出 / 扫描与文本识别等模式切换后视觉上「什么都没展开」有多因叠加，本轮修 AppShell 命令路由核心一项 + 核对 stub fail-closed 基础设施。`AppShell.tsx` `executeCommand` 原 `else if (group==="export"‖"forms"‖(mode 非 annotate)) → onUtilityPanelChange("none")` 把进入 export/forms/编辑/OCR 等模式的命令**一律把左侧 utility panel 强关到 none**——其中 forms 尤甚（forms 无右栏，唯一功能展开面是左栏 FormsPanel，被强关后只剩 L4 工具条）。现改为 mode-aware：裸 `mode→forms` 命令（group="mode" targetMode="forms" 无 targetUtilityPanel）显式 `onUtilityPanelChange("forms")` 展开 FormsPanel；export / ocr / pages / edit / read 不再强关左栏（ocr/pages 左栏由 `showUtilityPanel` 自然隐藏；export 主区 ExportDeliveryPanel + 右栏 export-preview、ocr 右栏 ocr-queue 由 `defaultRightPanel` 按 activeMode 派生，本就正确展开；edit 左栏大纲 summary 有意保留；read 阅读态本应见 summary）。带 `targetUtilityPanel` 的命令（forms-flatten / annotations-flatten / export-set/remove-password 等）走第一分支不受影响。stub fail-closed 核对结论：pure stub（批注辅助 9 项 / ocr-enhance-all / view-toolbar·sidebar-toggle / pdf-edit-* 5 项）已通过 commands.ts `availability:"planned"` + `executeCommand` 早返回（`「label」尚未接入真实功能，当前不可用`，不触发 mode/panel 副作用）实现 fail-closed（`AppShell.test.tsx` pdf-edit-content 用例已断言），ContextToolbar edit 模式 4 按钮已是 disabled + `title` + 状态文案的 fail-closed 模范——AppShell 侧本轮无需再改；完整 disabled+tooltip 渲染态在 Toolbar（按 availability 渲染）留后续 ISS。`npm run typecheck` 通过；GUI 实机走查留用户。
...
- **导出页数正确**：粘贴的副本页经 pdf-lib `copyPages` 真实写入新 PDF，导出重开页数 +1、副本页内容与源页一致。
- **M3 纵向闭环完成**：真实缩略图、选择/多选、拖拽、删除、旋转、撤销、导出重开、复制/粘贴全部 behavior-complete；剩余为真实页面尺寸标签、响应式断点和历史 skeleton 清理（视觉优化）。

## 未发版 · M5 异常态收尾：权限不足 + OCR 失败（2026-07-30，DEC-194）

- **权限不足可复现**：打开无读权限 / 不存在的 PDF 时不再只 console.error 或吞掉错误，`read_pdf_file_from_path` 迁移到 `Result<_,AppError>`，按 `io::ErrorKind` 自动区分「文件不存在 / 权限不足 / 其他 IO 错误」，前端走归一化中文错误卡片（复用 DEC-192），路径脱敏不泄露完整路径。
- **OCR 失败可验证**：补 `OcrDispatchError` 7 个失败变体（工具缺失 / 启动失败 / 进程失败 / 网络失败 / 凭证缺失 / 响应解析失败等）的 Rust 单测覆盖；此前 E2E 工具缺失时只跳过不断言失败，现纯构造测试在 CI 无工具环境稳定通过。
- **M5 主线收口**：文件损坏（DEC-192）+ 密码 PDF（DEC-193）+ 权限不足 / OCR 失败（DEC-194）四项异常态全部 behavior-complete 并合并（PR #70/#72/#73/#74）。
- **编排改进**：本轮两项用独立 worktree 隔离并行推进（`.claude/worktrees/m5-ocr-failure` + `m5-permission-denied`），文件零重叠，避免主工作区并发抢占。
...
> **supersede 关系**：本节保留 DEC-185 历史决策记录供追溯；几何 / 栏宽 / 验证器基线相关条目已被 DEC-186 取代（M2.1 阶段二次像素校准 + 73 项门禁）。状态机拆分等其余决定继续有效。
