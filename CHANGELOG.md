...
### Known Issues
- **阅读区暂无 pdfjs 文字层 DOM → 文字无法选中/复制（ISS-QA-18，待开工）**：当前阅读区只渲染 canvas（位图）+ 文字层状态徽标；`TextSelectionToolbar` / `usePdfTextSelection`（ISS-061）监听的选区依赖 pdfjs 文字层 span，而该层从未在阅读区渲染——真机「内容没法选中和复制」即此。修复方向：ReaderCanvas 按 viewport 渲染 pdfjs `TextLayer` 透明 span 层（对齐、缩放、性能与 canvas 同步）。
