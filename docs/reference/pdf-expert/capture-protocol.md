...
| 主题 | macOS 浅色（Light） | 采集前确认系统外观为浅色；深色作为 M2+ 另行采集 |
...
5. 触发目标状态（见下表）
6. screencapture -x -m -R 捕获 app 窗口区域（或全屏后 sips crop）
7. sips 裁剪到窗口区域，输出 G<NN>-<state>-<run>.png
...
- 工具：Preview/像素尺读取 crop 图像素坐标，按 Retina 2× 还原为逻辑 pt。
- 每个 key surface 记录：`{x, y, w, h}`（逻辑 pt，相对窗口左上原点）、`font_size_pt`、`gap_pt`、`method`、`uncertainty_pt`（人工量测默认 ±2pt）。
