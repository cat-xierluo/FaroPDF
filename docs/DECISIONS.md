...
- **ISS-030 工具栏布局克制化**（P1，UI/布局，已完成 2026-06-07）— 一级/二级分层、侧边栏默认关闭、页面布局收左上角。
- **ISS-031 PDF 打开/渲染功能修复**（P1，缺陷修复，已完成 2026-06-07）— 拖拽打开 + 文件选择后渲染异常修复。
- **ISS-032 macOS 菜单栏中文化 + 深层功能菜单入口**（P2，UI/本地化，已完成 2026-06-08）— 中文菜单 + 原生菜单桥接补齐。
- **ISS-033 主页下半部分灰色渲染异常**（P1，缺陷修复，已完成 2026-06-07）— 空态背景与 dropzone 视觉一致。
- **ISS-034 欢迎页"最近文件"硬编码占位符移除**（P2，UI/缺陷修复，已完成 2026-06-07）— 无真实最近文件时隐藏占位。
- **ISS-035 设置页 UI 视觉统一**（P2，UI/视觉，已完成 2026-06-07）— 高光/亮色不一致修复。
- **ISS-037 工具栏左上角品牌区域去除图标和名称占位**（P2，UI/布局，已完成 2026-06-07）— 让位页面布局功能。
- **ISS-038 DESIGN.md 对齐 Folia/Funes 设计系统成熟度**（P2，设计系统，已完成 2026-06-07）— 补全色彩/字体/布局/组件/交互/密度/深度/禁止事项规范。
- **ISS-039 PDF 工具启动器与导出编号面板收口**（P0，UI 信息架构/导出，已完成 2026-06-08）— 工具菜单四组分组 + 页码/Bates 进导出模式交付面板，默认另存新副本。
- **ISS-040 文字/图片水印交付面板接入**（P1，导出 UI，已完成 2026-06-08）— ExportDeliveryPanel 支持文字/图片水印，输出新副本。
- **ISS-041 压缩交付面板接入**（P1，导出 UI/压缩，已完成 2026-06-08）— 压缩预设选择 + apply 模式真实替换工作 PDF。
- **ISS-042 页眉页脚交付面板接入**（P1，导出 UI/页眉页脚，已完成 2026-06-08）— 页眉/页脚/字号/透明度，复用文字 watermark operation。
- **ISS-043 表单扁平化入口与填写签名工具条收口**（P1，UI/表单，已完成 2026-06-08）— forms-flatten 路由 + 填写签名工具条去占位。
- **ISS-044 页面管理工作台真实状态与另存导出接入**（P0，页面整理，已完成 2026-06-08）— pageOrganizer 状态机接入 + 另存 `*-organized.pdf`。
- **ISS-045 批注扁平化入口与批注侧栏导出接入**（P0，批注，已完成 2026-06-08）— annotations-flatten + 批注侧栏扁平化导出 `*-annotations-flattened.pdf`。
- **ISS-046 页面管理重排 UI 与真实另存接入**（P0，页面整理，已完成 2026-06-08）— 上移/下移 + 撤销 + 重排参与另存导出。
- **ISS-047 ReaderCanvas PDF.js 并发渲染取消缺陷**（P1，阅读渲染/缺陷修复，已完成 2026-06-08）— 取消上一轮 render task，避免同 canvas 并发。
- **ISS-048 工具启动器去假入口与批注摘要重路由**（P0，UI 信息架构，已完成 2026-06-08）— 空态去转换假入口 + 另存为真实副本 + 批注摘要路由侧栏。
- **ISS-049 阅读模式工具注册幂等修复**（P0，UI/缺陷修复，已完成 2026-06-08）— toolbarRegistry 按 mode+id 去重。
- **ISS-050 原生菜单系统动作去提示型假入口**（P0，UI/原生菜单，已完成 2026-06-08）— 新建窗口/关于走真实系统动作，Rust 菜单事件处理。
- **ISS-051 页眉页脚奇偶页范围接入**（P1，导出 UI/页眉页脚深化，已完成 2026-06-09）— 全部/奇数页/偶数页范围 + pageIndexes 精确传递。
- **ISS-052 缩略图状态标记克制化**（P1，UI/左侧摘要，已完成 2026-06-09）— 批注/命中/OCR 改紧凑视觉标记。
- **ISS-053 页眉页脚视觉位置选择器**（P1，导出 UI/页眉页脚深化，已完成 2026-06-09）— 左/中/右位置选择器 + 奇偶页范围。
- **ISS-054 深色模式最小接入**（P1，UI 视觉/设置，已完成 2026-06-09）— AppSettings 外观偏好 + data-theme + 深色 token。
- **ISS-055 顶栏任务模式入口收口**（P0，UI 信息架构，已完成 2026-06-09）— 任务模式入口统一进工具启动器，顶栏去四按钮。
## 工作日志
...
- **新证据**：`npm run tauri dev` 跑最新 main（含 workerPort 修复 + DEV 诊断），自动加载 `tests/fixtures/expert/reference.pdf` → PDF **正常打开**，UI 显示「第 1 页 / 1033×1457 / **文字层未知**」。截图存档为 `faropdf_with_devtools.png`（修复前同会话截图）。修正 DEC-196 原文第三段误判。
- **真因**：workerPort 修复**有效**（PDF 解析、pageCount=5 正常 = worker 链通）。`textLayerStatus=unknown` 来自 `readTextLayerStatus` 调 `page.getTextContent()` 时真机抛 `UnknownErrorException: Ensure that the standardFontDataUrl API parameter is provided`，被 `pdfReaderService.ts:112` catch 吞错。e2e jsdom 下用 legacy fake worker 不走字体路径所以 PASS；真机 WKWebView 必须传。
- **修复（第 2 版，替换初版）**：初版 `standardFontDataUrl: new URL("pdfjs-dist/standard_fonts/", import.meta.url).href` 在 vite build 后解析为 `tauri://localhost/assets/pdfjs-dist/standard_fonts/`（vite 不把 bare module 目录当资产）→ **404 无效**。改由 `config/vite.config.ts` 新增 `provideStandardFonts` plugin：dev 用中间件把 `/standard_fonts/*` 映射到 node_modules 真实文件；build `closeBundle` 复制 16 个字体到 `dist/standard_fonts/`。代码侧 `standardFontDataUrl: "/standard_fonts/"`（根路径，dev/prod 一致）。已验证 `dist/standard_fonts/` 有 16 文件 + `curl /standard_fonts/FoxitFixed.pfb` 返回 200 TTF。
- **真机二次验证（chromium 真 Worker）**：新增 `scripts/qa02-verify.mjs`（Playwright chromium + vite dev，真 module worker + 真 fetch `/standard_fonts/*`，行为与 WKWebView 一致）。结果：reference.pdf `textLayerStatus="available"`、pageCount=5、firstPageChars=422；corrupt.pdf 抛 `InvalidPDFException`。**PASS**——standardFontDataUrl 修复在真 Worker 模式下有效。
