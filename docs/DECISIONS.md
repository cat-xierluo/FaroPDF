...
- **修复**：`defaultAdapter.getDocument` 增 `standardFontDataUrl: new URL("pdfjs-dist/standard_fonts/", import.meta.url).href`，与 `pdfjsWorker.ts` 的 worker URL 同样的 Vite 资源处理。E2E 7/7 PASS（无 "Ensure that the standardFontDataUrl" warning + reference.pdf `textLayerStatus≠unknown` 断言保持）。
- **真机二次验证状态**：`npm run tauri dev` 重启后 DEV 诊断未在 UI 反映（Vite HMR + StrictMode 干扰 + `document.title` 不映射到 Tauri 窗口标题）—— 实机无 devtools console 抓取（Safari 开发菜单未列出 FaroPDF WKWebView target，疑似 inspectable 未开），决定**不**通过改 main.tsx 注入持久 DOM 显示 / 硬编码路径重抓。**当前实机验证是「修复前『文字层未知』」截图证据 + e2e jsdom 下修复后 PASS 推断**。
- **完成度**：QA-02 修复代码已落 e2e 7/7 PASS；真机二次实拍取证未完成（need inspectable 开启 / Safari 列出 / 或真机 dev 日志），**留作下一轮真机验证的「拍快照到 state 文件」issue**。用户实机跑 build 产物应可见「文字层：可检测」替代「未知」（前提：`pdfjs-dist/standard_fonts/` 在 build bundle 中被 vite 正确 emit——需用真 build + 截图确认）。
