...
- 下一步：真实 PDF 端到端 fixture 验证（PR #33 / DEC-050 已落端到端测试基础设施 + 1 case；多 fixture 多 case 持续补充）、根据 `legal-skills` 实际可用脚本收敛 fallback 逻辑。`keychain:` 凭证引用 + OS Keychain 集成已在 `feat/iss-007-keychain` 完成（DEC-059），待 PM 合 review。
