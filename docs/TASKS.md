...
| ISS-QA-02 | 打开显示「未知」+ 慢 + 缩略图不可用 | [P0] | 三层剥洋葱：①workerPort 修打开失败（`tauri://` origin null）②standardFontDataUrl 修「文字层未知」（getTextContent 抛 UnknownErrorException）③vite plugin emit 字体（第 1 版 bare module 打包 404） | `pdfjsWorker.ts` + `pdfReaderService.ts` + `config/vite.config.ts` | behavior-complete | [2026-08-14] standardFontDataUrl 第 2 版：`provideStandardFonts` plugin（dev 中间件 + build 复制 16 字体）替代 `acc488e` 第 1 版（bare module → 404）；PM 独立复跑（非自报）：chromium 真 worker `textLayerStatus=available` + `dist/standard_fonts/` 16 文件 + e2e 7/7 + lint 0 error。**WKWebView 真机实拍 NOT_VERIFIED**（Safari 列不出 target），留用户 `tauri build` 后确认「文字层：可检测」 |
