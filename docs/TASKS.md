...
- 验收：
  ...
  - 顺带修复 `extract_pdf_text` 的 pdftotext 参数顺序 bug（详见 DEC-048 §2.4）
- 当前进度：
  ...
  - 总测试 74 文件 / 697 tests 通过；typecheck / build 失败是项目级 pre-existing `target: ES2020` 不支持 `Array.prototype.at`（与本 PR 无关，详见 DEC-048 §4 / §5）
  - `docs/DECISIONS.md` 追加 DEC-048（`feat/ocr-e2e` 整体方案 + scope 变更说明 + bug fix 归并 + typecheck 现状）
  - 进度日志：本任务卡（即此段）+ `CHANGELOG.md` 新增 0.1.0-alpha.10 段（合并 DEC-048 的 OCR E2E 进展）
