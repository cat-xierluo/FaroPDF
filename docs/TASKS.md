...
- 2026-06-04：在 `feat/ocr-e2e` 推进 ISS-007 端到端联调（DEC-050）：新增 `tests/fixtures/ocr/generate-scan-fixture.mjs` + `README.md`（pdf-lib 嵌入 base64 PNG 生成 2 页 A4 扫描件 ~5 KB，`.gitignore` 排除）+ `tests/e2e/ocr-e2e.test.ts`（4 case：full pipeline 真实 ocrmypdf + pdftotext 端到端 / bridge rejects mismatched providerId / controller sanitises backend errors / prepareOcrRequest defaults）+ `src-tauri/src/lib.rs` 末尾 `ocr_e2e_tests` mod（1 case：dispatch_ocr + extract_pdf_text + OcrJobQueue 持久化端到端）；顺手修 `src-tauri/src/ocr_text_extract.rs` 的 pdftotext 参数顺序 bug（之前 `.arg("-").arg(pdf_path)` 顺序与 pdftotext CLI 语法不符，导致 `Syntax Error: Document stream is empty`；修正为 `.arg(pdf_path).arg("-")`；E2E 真实链接测试补了这个盲点）。总测试 74 文件 / 697 通过；`cargo test --lib` 42 / 42 通过；`cargo check` 干净（9 个 pre-existing dead_code warning 来自 scan_preprocess）；`npx vite build` ✅（2.81s 出完整 dist）；`npm run build` 走 `tsc && vite build` 阶段因项目级 pre-existing `target: ES2020` 不支持 `Array.prototype.at` 报 28 个 TS2550 错误（**与本 PR 无关**，详见 DEC-050 §4）；scope 变更（`src-tauri/src/lib.rs` 从 forbidden 移到 allowed）已在 STATUS.json `scope_change_log` 详细说明并设 `pm_action_required=true`；`package.json` / 锁文件 / `src-tauri/Cargo.toml` / `Toolbar.tsx` / `App.tsx` / `src/components/**` / `src/shared/ocr/*` 契约 / 其他模块（reader / search / annotation / forms / export / pages / settings / scan-preprocess）**未修改**；不引入新 crate / 新 npm 包。
- 2026-06-04：在 `feat/ocr-e2e` 推进 ISS-007 端到端联调（DEC-050）：新增 `tests/fixtures/ocr/generate-scan-fixture.mjs` + `README.md`（pdf-lib 嵌入 base64 PNG 生成 2 页 A4 扫描件 ~5 KB，`.gitignore` 排除）+ `tests/e2e/ocr-e2e.test.ts`（4 case：full pipeline 真实 ocrmypdf + pdftotext 端到端 / bridge rejects mismatched providerId / controller sanitises backend errors / prepareOcrRequest defaults）+ `src-tauri/src/lib.rs` 末尾 `ocr_e2e_tests` mod（1 case：dispatch_ocr + extract_pdf_text + OcrJobQueue 持久化端到端）；顺手修 `src-tauri/src/ocr_text_extract.rs` 的 pdftotext 参数顺序 bug（之前 `.arg("-").arg(pdf_path)` 顺序与 pdftotext CLI 语法不符，导致 `Syntax Error: Document stream is empty`；修正为 `.arg(pdf_path).arg("-")`；E2E 真实链接测试补了这个盲点）。总测试 74 文件 / 697 通过；`cargo test --lib` 42 / 42 通过；`cargo check` 干净（9 个 pre-existing dead_code warning 来自 scan_preprocess）；`npx vite build` ✅（2.81s 出完整 dist）；`npm run build` 走 `tsc && vite build` 阶段因项目级 pre-existing `target: ES2020` 不支持 `Array.prototype.at` 报 28 个 TS2550 错误（**与本 PR 无关**，详见 DEC-050 §4）；scope 变更（`src-tauri/src/lib.rs` 从 forbidden 移到 allowed）已在 STATUS.json `scope_change_log` 详细说明并设 `pm_action_required=true`；`package.json` / 锁文件 / `src-tauri/Cargo.toml` / `Toolbar.tsx` / `App.tsx` / `src/components/**` / `src/shared/ocr/*` 契约 / 其他模块（reader / search / annotation / forms / export / pages / settings / scan-preprocess）**未修改**；不引入新 crate / 新 npm 包。
- 2026-06-05：在 `feat/iss-023-author-update` 推进 ISS-023 第一版收口（DEC-051）：ISS-023 任务卡要求分两块——「关于」section 元数据 + 检查更新（DEC-038 + DEC-048 已落，本期不重做）+ 作者卡（DEC-038 占位是 footnote，**未**有真实二维码 / 扫码说明 / 独立组件）。本期收口作者卡：(a) `[m1] feat(settings): AuthorCard 基础组件` 新增 `src/components/settings/AuthorCard.tsx`（受控组件，props 注入 `authorName` / `githubUrl` / `wechatQrSrc` / `wechatQrAlt` / `scanInstruction` / `className`；空 `authorName` 兜底显示「作者信息未配置」）+ `AuthorCard.test.tsx`（6 项：name / GitHub href-target-rel / QR src-alt / 扫码说明 / 空 name 兜底 / className 透传）+ `AuthorCard.css`（独立样式，复用 `SettingsPanel.css` 的 `.settings-author-card` 基础壳层，扩展 `__name / __author-name / __github-link / __qr / __qr-image (image-rendering: pixelated 让 1×1 占位图保持方块感) / __instruction`；`@media (max-width: 479px)` 折叠为单列避免 900px 视口下文字被挤压）+ `src/assets/wechat-qrcode.png`（1×1 灰阶 PNG 占位，67 字节，Python 直接写 PNG 三 chunk 生成，**不**依赖 ImageMagick / opencv）+ `src/assets/QRCODE_LICENSE.md`（说明当前为占位、后续替换流程、**不**收录账号 / 密码 / Token / 私钥、仅使用项目作者本人拥有或经授权的二维码）。(b) `[m2] feat(settings): AboutSection 接 AuthorCard，替换占位`：`src/modules/settings/sections/AboutSection.tsx` 末尾 `<div className="settings-author-card">作者卡暂为占位……</div>` 替换为 `<AuthorCard authorName={metadata.authorName ?? ""} githubUrl={metadata.repositoryUrl ?? "https://github.com/cat-xierluo"} wechatQrSrc={wechatQrUrl} />`；新增 2 个 import（AuthorCard 组件 + `wechat-qrcode.png` 静态资源）；`AboutSection.test.tsx` 追加 3 项（QR 图片 alt/src 含「微信」+「wechat-qrcode」、扫码说明含「微信扫码/关注」、legacy placeholder 消失）。**未**改 `src/components/layout/{Toolbar,Sidebar}.tsx` / `App.tsx` / `src/styles/app.css` / `package.json` / 锁文件 / `src-tauri/**` / `src/shared/settings/**` / `src/shared/update/**` / `src/shared/app/metadata.ts`（AuthorCard 透过 props 注入，不直接耦合 metadata）/ 其他模块（reader / search / annotation / forms / export / pages / ocr / preprocess）；未引入新依赖。新增 9 项测试（AuthorCard 6 + AboutSection 3），总测试 76 文件 / 703 通过（typecheck / build / cargo check --offline 全绿）。`docs/DECISIONS.md` 追加 DEC-051；`CHANGELOG.md` 新增 0.1.0-alpha.11 段；ISS-023 任务卡状态从「待处理」推到「第一版收口（待 PM 合 review）」，「下一步」改为「公众号二维码替换为真实图片（按 QRCODE_LICENSE.md 流程）+ 与 PM 协同 PR 合并 / 视觉验收」。已知限制：当前二维码是 1×1 占位图（`image-rendering: pixelated`），正式发布前必须由作者替换为真实公众号二维码；公众号二维码**不**支持热更新；AuthorCard 不感知 dark mode（仅跟随全局 CSS variable）。
