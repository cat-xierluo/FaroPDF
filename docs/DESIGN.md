...
│ Title Bar / Toolbar                          │
├──────────────┬───────────────────┬───────────┤
│ Sidebar      │ PDF Pages          │ Inspector │
│ Thumbnails   │ Virtual viewport   │ Search    │
│ Outline      │ Text + annotation  │ Notes/OCR │
├──────────────┴───────────────────┴───────────┤
...
- 左侧：打开、另存、导出、最近文件。
- 中间：页码、缩放、适合宽度、单页/连续/双页。
- 右侧：搜索、批注、页面整理、OCR、设置。
- 图标优先，必要时使用短标签。
- 批注工具条可在进入批注模式后显示，不常驻挤压阅读区。
...
右侧面板按任务切换，不同时常驻多个复杂面板：

- 搜索：关键词输入、命中列表、上下文。
- 批注：批注列表、筛选、导出摘要。
- OCR：文字层状态、后端选择、页码范围、任务进度。
- 页面：缩略图网格、旋转、删除、重排、插入、提取、合并。
- 表单：字段列表、签名工具、扁平化导出。
